...
let PromptTemplatesComponent = class PromptTemplatesComponent {
    constructor(sanitizer, http, currencyService, hawkAgentService, promptTemplatesService, cdr) {
        ...
        this.msgUidCounter = 21; // Start from 21 as database has up to 20
        this.instructionIdCounter = 21; // Start from 21 as database has up to 20
        this.streamBuffer = '';
        // Enhanced features
        ...
        this.agentUrl = AGENT_IFRAME_URL;
        this.agentOrigin = AGENT_IFRAME_ORIGIN;
        ...
        this.onMessage = (event) => {
            ...
            if (this.agentOrigin !== '*' && event.origin !== this.agentOrigin)
                return;
        ...
        // Form controls
        this.currencies = []; // Will be fetched from currency config table
        this.currency = 'SGD';
        this.amount = null;
        this.date = new Date().toISOString().split('T')[0]; // Default to today
        this.scopes = ['Branch', 'Subsidiary', 'Associate', 'TMU'];
        this.subsidiaryLevels = ['Entity', 'Subsidiary', 'Branch'];
        this.riskFactors = ['FX Rate', 'Interest Rate', 'Commodity', 'Equity'];
        this.genericOptions = ['Option A', 'Option B', 'Option C'];
        this.placeholderValues = {};
        this.trackByIndex = (_, __) => _;
        this.trackByKey = (_, item) => item.key;
        this.trackByPart = (_, part) => part.type + ':' + (part.text ?? part.name ?? _);
    }
    ...
    resetIdCounters(msgUidStart = 21, instructionIdStart = 21) {
    ...
    updateTemplatesFromDatabase(templates) {
        // Reset everything
        this.allConvertedTemplates = [];
        this.categories = [];
        this.instructionTypes = [];
    ...
    // Removed - no longer needed with simplified approach
    getFamilyTypeForInstructionType(instructionType) {
        // Map instruction types back to family types
        const instructionToFamily = {
    ...
    refreshPromptsForCategory() {
        ...
        console.log(`✓ Updated category "${this.activeCategory}" -> ${this.currentPrompts.length} templates:`, this.currentPrompts.map(p => p.templateName));
        // Force change detection immediately
        this.cdr.detectChanges();
        // Calculate real usage stats from database
        this.calculateRealUsageStats();
    }
    get selectedPromptBase() {
    ...
    initPlaceholdersForSelectedPrompt(resetNonAmount = false) {
        const base = this.isSelectedTemplateDynamic()
            ? this.getSelectedTemplateText()
            : this.selectedPromptBase;
    ...
    async createDatabaseSession(promptText) {
        try {
            ...
            await this.hawkAgentService.createSession(promptText, this.currentMsgUid, this.currentInstructionId, this.activeCategory, this.selectedPromptIndex + 1 // Convert to 1-based index
            );
    ...
    async updateDatabaseSession(status, conversationId, taskId, tokenUsage) {
        try {
    ...
    finishStream(conversationId, taskId, tokenUsage) {
    ...
    isFormValid() {
        ...
        // Handle static templates (existing logic)
        const placeholderParts = this.previewParts.filter(p => p.type === 'placeholder');
    ...
    getPlaceholderParts() {
        return this.previewParts.filter(p => p.type === 'placeholder');
    }
    getCurrentDateTime() {
    ...
    getResultStatus() {
        ...
        if (response.includes('approved') || response.includes('confirmed'))
            return 'Approved';
        if (response.includes('rejected') || response.includes('denied'))
            return 'Rejected';
        if (response.includes('pending') || response.includes('review'))
    ...
    addToRecentTemplates() {
        ...
        this.recentTemplates = this.recentTemplates.filter(r => !(r.category === recent.category && r.index === recent.index));
        // Add to beginning
        this.recentTemplates.unshift(recent);
        // Keep only last 5
        this.recentTemplates = this.recentTemplates.slice(0, 5);
    }
    loadRecentTemplate(recent) {
    ...
    updateTemplateMetricsFromCompletion(status) {
    ...
    updateMasterTemplateData(_type) {
    ...
    onInstructionTypeSelect(key) {
        if (this.activeInstructionType === key)
            return;
    ...
    getCategoryTabClass(categoryKey) {
    ...
    getSelectedInstructionTypeLabel() {
        ...
        try {
            ...
            this.currentPrompts.forEach((prompt, index) => {
                ...
                const successfulSessions = matchingSessions.filter(session => {
                    ...
                    if (userMarkedComplete)
                        return true;
                    if (userMarkedIncomplete)
                        return false;
                    return isCompleted;
                });
    ...
    getSelectedTemplateInputFields() {
        if (this.selectedPromptIndex < 0 || !this.currentPrompts[this.selectedPromptIndex]) {
            return [];
        }
        const text = this.getSelectedTemplateText();
    ...
    onDynamicInputChange() {
        ...
        // The getFilledTemplate() method will be called automatically by Angular change detection
    }
    getFilledTemplate() {
        if (this.selectedPromptIndex < 0 || !this.currentPrompts[this.selectedPromptIndex]) {
            return '';
        ...
        Object.keys(this.dynamicInputValues).forEach(key => {
            ...
            filledText = filledText.replace(regex, this.dynamicInputValues[key] || `{{${key}}}`);
        });
        return filledText;
    }
...
__decorate([
    ViewChild('agentFrame')
], PromptTemplatesComponent.prototype, "agentFrameRef", void 0);
__decorate([
    ViewChild('resultsSection')
], PromptTemplatesComponent.prototype, "resultsSectionRef", void 0);
PromptTemplatesComponent = __decorate([
    Component({
...
export { PromptTemplatesComponent };
//# sourceMappingURL=prompt-templates.component.js.map