...
let TemplatePreviewComponent = class TemplatePreviewComponent {
    send() {
    ...
    getType(f) {
        ...
        if (name.includes('date'))
            return 'date';
        if (name.includes('amount') || name.includes('qty') || name.includes('quantity') || name.includes('rate'))
            return 'number';
        return 'text';
    }
    getPlaceholder(f) {
        ...
        if (name.includes('currency'))
            return 'e.g., USD';
        if (name.includes('amount') || name.includes('qty') || name.includes('quantity'))
            return 'e.g., 100000';
    ...
    getSelectOptions(f) {
        ...
        if (name === 'entity' || name.includes('entity_id') || name.includes('entity_code') || name.includes('entity_name'))
            return this.entityOptions;
        return [];
    }
    constructor(currencySvc, entitySvc) {
    ...
    ngOnInit() {
        ...
        try {
            this.currencySvc.getCurrencyCodes().subscribe(list => this.currencyOptions = list || []);
        }
        catch { }
        // Load entity options
        try {
            this.entitySvc.entities$.subscribe((ents) => {
...
__decorate([
    Input()
], TemplatePreviewComponent.prototype, "template", void 0);
__decorate([
    Input()
], TemplatePreviewComponent.prototype, "fields", void 0);
__decorate([
    Output()
], TemplatePreviewComponent.prototype, "onSend", void 0);
__decorate([
    Input()
], TemplatePreviewComponent.prototype, "responseText", void 0);
TemplatePreviewComponent = __decorate([
    Component({
...
//# sourceMappingURL=template-preview.component.js.map