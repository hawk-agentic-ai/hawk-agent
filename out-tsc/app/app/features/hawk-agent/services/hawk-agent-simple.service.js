...
let HawkAgentSimpleService = class HawkAgentSimpleService {
    // Create a new session
    async createSession(promptText, msgUid, instructionId, templateCategory, templateIndex) {
        const payload = {
            msg_uid: msgUid,
    ...
    // Update session
    async updateSession(msgUid, updates) {
        try {
            const supabase = getSupabase();
...
HawkAgentSimpleService = __decorate([
    Injectable({
        providedIn: 'root'
    })
], HawkAgentSimpleService);
export { HawkAgentSimpleService };
//# sourceMappingURL=hawk-agent-simple.service.js.map