...
let DashboardService = class DashboardService {
    constructor() {
        ...
        this._metrics = new BehaviorSubject(null);
    ...
    async refreshWithCurrencyFilter(currencyFilter) {
        this.currentCurrencyFilter = currencyFilter;
        await this.refresh(currencyFilter);
    }
    async refresh(currencyFilter) {
        ...
        const navPositionByDate = new Map();
        rows.forEach(r => {
            const d = r.as_of_date ? new Date(r.as_of_date) : null;
            if (!d)
                return;
