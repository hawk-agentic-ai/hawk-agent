...
import { BufferConfigurationService, BufferConfiguration } from './buffer-configuration.service';
import { EntityMasterService, EntityMaster } from '../entity/entity-master.service';
import { HedgingFrameworkService, HedgingFrameworkRow } from '../hedge-framework/hedging-framework.service';
...
export class BufferConfigurationComponent implements OnInit, OnDestroy {
  ...
  columnDefs: ColDef[] = [
    ...
    { field: 'active_flag', headerName: 'Status', width: 110, cellRenderer: (p: any) => {
  ...
  // Data/state
  rows: BufferConfiguration[] = [];
  search = '';

  // Dialog state
  showDialog = false;
  mode: 'add' | 'edit' = 'add';
  form: Partial<BufferConfiguration> = {};

  constructor(
    private ngZone: NgZone,
    private svc: BufferConfigurationService,
    private entitySvc: EntityMasterService,
    private frameworkSvc: HedgingFrameworkService
  ) {}

  // Dropdowns
  entityTypeOptions: { label: string, value: string }[] = [];
  entityIdOptions: { label: string, value: string }[] = [];
  private entitiesSub?: any;
  private latestEntities: EntityMaster[] = [];
  hedgingFrameworkOptions: { label: string, value: string }[] = [];
  private frameworksCache: HedgingFrameworkRow[] = [];
  ...
  ngOnInit() {
    ...
    this.entitiesSub = this.entitySvc.entities$.subscribe((entities: EntityMaster[]) => {
      ...
      if (this.form.entity_type) {
        this.refreshEntityIds(this.latestEntities, this.form.entity_type as string);
      }
    });
  ...
  ngOnDestroy() {
    ...
    if (this.entitiesSub) this.entitiesSub.unsubscribe();
  }
  ...
  applyFilters() {
    ...
    if (this.search.trim()) {
      ...
      filtered = filtered.filter(t =>
        (t.buffer_rule_id?.toLowerCase().includes(s) ||
  ...
  editRow(row: BufferConfiguration) {
    this.mode = 'edit';
    this.form = { ...row };
    this.ngZone.run(() => this.showDialog = true);
  }

  async deleteRow(row: BufferConfiguration) {
    if (!row?.buffer_rule_id) return;
  ...
  onEntityTypeChange(value: string) {
    ...
    this.form.entity_id = '' as any;
    // Refresh framework options (optionally filter by entity)
    this.refreshFrameworkOptions();
  }

  onEntityChange(value: string) {
    this.form.entity_id = value as any;
    this.refreshFrameworkOptions();
  }

  private refreshEntityIds(entities: EntityMaster[], type?: string) {
  ...
  private async loadFrameworkOptions() {
    try {
  ...
  private refreshFrameworkOptions() {
    const entityId = this.form.entity_id as string | undefined;
