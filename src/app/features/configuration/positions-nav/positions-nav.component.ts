...
import { PositionsNavService, PositionNavRow } from './positions-nav.service';
...
export class PositionsNavComponent implements OnInit {
  rows: any[] = [];
  search = '';
  showSheet = false;
  mode: 'add'|'edit' = 'add';
  ...
  form: any = { nav_id: '', entity_id: '', currency_code: '', nav_type: '' };
  ...
  constructor(private zone: NgZone, private svc: PositionsNavService, private currencySvc: CurrencyService) {}
  ngOnInit(){
    this.loadRows();
    this.loadCurrencyOptions();
  }

  private async loadCurrencyOptions(){
    try {
  ...
  async editRow(row: PositionNavRow){
    this.mode='edit';
  ...
  deleteRow(row: PositionNavRow){ this.pendingDeleteId=row.nav_id; this.showConfirmDelete=true; this.showSheet=false; }
  async confirmDelete(){
    if (!this.pendingDeleteId) return; await this.svc.deleteById(this.pendingDeleteId);
    this.showConfirmDelete=false; this.pendingDeleteId=null; await this.loadRows();
  }

  async confirmSave(){
    if (this.mode === "edit") {
      await this.svc.updateById(this.form.nav_id, this.form as any);
    } else {
      await this.svc.insert(this.form as any);
    }
    this.showConfirmSave=false; this.showSheet=false; this.mode="add"; await this.loadRows();
  }
}
