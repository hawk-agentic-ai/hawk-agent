...
import { HedgingFrameworkService, HedgingFrameworkRow } from './hedging-framework.service';
...
export class HedgeFrameworkComponent implements OnInit {
  frameworks: any[] = [];
  showSheet = false;
  ...
  columnDefs: ColDef[] = [
    ...
    { field: 'notes', headerName: 'Notes', width: 220, cellRenderer: (p:any)=>{ const d=document.createElement('div'); d.title=p.value||''; d.textContent=(p.value||''); return d; } },
  ...
  constructor(private svc: HedgingFrameworkService, private zone: NgZone) {}
  ...
  async editFramework(row: HedgingFrameworkRow) {
    ...
    this.framework = {
      ...
      created_date: (r as any).created_date || '',
      modified_by: (r as any).modified_by || '',
      modified_date: (r as any).modified_date || ''
    };
    this.zone.run(()=> this.showSheet = true);
  }
  deleteFramework(row: HedgingFrameworkRow) { this.pendingDeleteId = row.framework_id; this.showConfirmDelete = true; this.showSheet = false; }
  async confirmSave() {
    const p = { ...this.framework } as Partial<HedgingFrameworkRow>;
    if (this.dialogMode === 'edit') {
      await this.svc.updateById(this.framework.framework_id, p);
    } else {
      // For add, require an ID and insert
      await this.svc.insert(p as HedgingFrameworkRow);
    }
    this.showConfirmSave = false; this.showSheet = false; this.dialogMode = 'add'; this.resetForm(); await this.loadFrameworks();
  }
  async confirmDelete() { if (!this.pendingDeleteId) return; await this.svc.deleteById(this.pendingDeleteId); this.showConfirmDelete = false; this.pendingDeleteId = null; await this.loadFrameworks(); }
  resetForm(){
