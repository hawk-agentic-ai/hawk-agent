...
import { ThresholdConfigurationService, ThresholdConfiguration } from './threshold-configuration.service';
...
export class ThresholdConfigurationComponent implements OnInit, OnDestroy {
  ...
  form: Partial<ThresholdConfiguration> = {
  ...
  columnDefs: ColDef[] = [
    ...
    { field: 'automated_action_flag', headerName: 'Automated', width: 100, cellRenderer: (p: any) => {
  ...
  constructor(private ngZone: NgZone, private thresholdService: ThresholdConfigurationService) {}
  ...
  ngOnDestroy() {
    if (this.thresholdSub) this.thresholdSub.unsubscribe();
  }
  ...
  applyFilters() {
    ...
    if (this.search.trim()) {
      ...
      filtered = filtered.filter(t =>
        (t.threshold_id?.toLowerCase().includes(s) ||
  ...
  async editRow(row: ThresholdConfiguration) {
    this.mode = 'edit';
    this.form = { ...row };
    this.ngZone.run(() => this.showSheet = true);
  }
  deleteRow(row: ThresholdConfiguration) { this.pendingDeleteId = row.threshold_id; this.showConfirmDelete = true; this.showSheet = false; }
  async confirmDelete() {
